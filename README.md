...
- **委託はホスト非依存に書く**: skill 本文は役割語(`researcher` / `implementer` / `reviewer` / `checker`)で委託を書き、どのバックエンドの何で実行するかは 1 箇所に集約する。**解決の正本**(役割語の一覧・派生名の体系・属性軸・解決順・ホストでの解決・縮退)は [delegation-map.md](plugins/dev-workflow/skills/do-task/references/delegation-map.md) の 1 ファイルだけで、README や design はそれを参照する。**委託の語の移行は v4.0.0(2026-09-10)で完了**しており、以後の新規混入は `scripts/validate.py` が ERROR で止める。**未決として残るのは `.claude/` 配下の状態ファイルの扱いだけ**で、語彙・検査範囲・除外集合・到達時の実測は [docs/design.md](docs/design.md) §7-7
