...
- 周に MCP のツールを渡すと(`--mcp-config`)、許可リストに無いツールは確認に回り、許可の仲介が「扱わないツール」で拒否する。発見モードの refactor の分析は、MCP を渡さなければ Glob・Grep の経路で動く
...
- 周に MCP を既定では渡さない。`--mcp-config` を渡さなければ周に MCP は無く、refactor の分析は Glob・Grep の経路で動く
