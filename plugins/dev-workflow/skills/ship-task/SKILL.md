...
| 品質ゲート赤・スコープ縮小・レビュー未収束・完了処理の停止(改名先が既に在る等) | doc 同期の needs-user(PR の残課題に転記) |
