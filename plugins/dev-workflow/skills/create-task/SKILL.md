...
**一気通貫の提案**: 実装の分岐を左右する不確定点が残らず、依存タスクも解消されている場合は、完了報告の最後に「このまま実装 → doc 同期 → PR まで進めますか?(/ship-task)」を 1 行で提案する。不確定点が残る場合は提案せず、何を決めれば進めるかを示す。**/ship-task から呼ばれている場合はこの提案を出さない**(呼び出し元が続行判定を行うため。design §5-21)。
