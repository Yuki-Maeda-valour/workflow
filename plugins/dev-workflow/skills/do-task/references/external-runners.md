...
> 契約を変えるときは、このファイルと、レビュー用の `scripts/review-agent.sh`・`scripts/review-agent-selftest.sh`、および実装用の委託スクリプトとその自己テスト(#51-B で新設)の**計 5 点**だけを直せばよい(既定表の同期は回帰テストが照合する)。
...
- **implementer も外部委託の対象にする。** 宣言された環境では implementer の既定を外部にする(オプトインは維持。判定・保護・縮退は §12 が定める)。**実装用の既定表にエントリが無い間は、宣言しても下記の信頼モデル表の規則により受け付けられず、内蔵 implementer で動作する**(既定表の実装用エントリの追加は別スコープ)
...
`bash {do-task の}scripts/review-agent-selftest.sh` が、次をスタブだけで検証する(外部 CLI もネットワークも不要。**`review-agent.sh`・実装用スクリプト、またはこの仕様書を触ったら必ず実行する**):
...
**実装用スクリプトには別の自己テストがある**(決定 9: 共通部分の切り出しはせず独立したスクリプトとして新設する。既存 66 件がこのファイルの `sed` による変異テストに依存するため、共通化すると壊れる)。実装用スクリプトまたは §12 を触ったときは、そちらの自己テストも合わせて実行する(スクリプトと自己テストの具体は #51-B で新設する)。
...
- **判定 1・2** は §3 と同じ(存在・自ホスト判定)。この 2 段は外部 CLI を**起動しない**
...
**この表にはエントリを入れない**(#51-B のスコープ。既定表に実装用エントリが無い間は、§1 の信頼モデル表の規則により宣言が受け付けられず、内蔵 implementer で実行される。これは明示的で安全な中間状態である)。

**既知の限界(6 件。§12-4 の機密ガードの限界 8 つとは別のリスト)**:
...
**「終了コードが 0 以外、かつ ①②③ の作業ツリー比較または ④⑤ の git メタ比較のいずれかに変化がある → 引き継ぎ。どちらにも変化が無ければ、起動前失敗と同じ縮退」**(決定 26)。**終了コードでの場合分けはしない**(スクリプトは git を呼ばないため「作業ツリーに変更が残っているか」を単独で判定できない。判定主体と情報源を一致させる)。**新設する終了コードは 11(`no-writemode`)だけ**(§6 の終了コード表に定義を集約する。ここでは発火条件だけを扱う)。
...
**スクリプトが残すもの**: 解決後の起動コマンド・渡した `--cwd`・判定結果(1・2・3′・4′)・生出力。記録先は `.claude/reviews/implementer-{ランナー}-{タスク名}-iter{N}.md`(`--log-file` で指定。省略時は `implementer-{ランナー}-iter{N}.md` を自動採番)。**置き場が `.claude/reviews/` のまま**であることは既知の限界(§12-6)で、この置き場だけが §12-2 の比較から除外される。**呼び出し側(skill)が報告に残す項目は §12-5 が定義するので、ここでは参照に留める**(同じ事実を 2 箇所に書かない。design §7-2)。

**実装スクリプトの起動構文**(契約として固定する):

```
bash {do-task の}scripts/{実装用スクリプト} --runner <名前> --prompt-file <パス> --cwd <ディレクトリ> \
...
| `--prompt-file <パス>` | **必須** | 委託プロンプトのファイルパス。空・未指定は `usage`(exit 2)。**「argv に載せない」の範囲はスクリプトへの入力に限る** — **スクリプトへはプロンプト本文を argv で渡さずファイルで渡す**が、**スクリプトから外部 CLI へはプロンプトが argv に載る**ため §9-2 の限界③(`ps` / `/proc/<pid>/cmdline` から他ユーザーに見えうる)はそのまま適用され、同じ理由で argv 上限(128KiB)に近いときは `prompt-too-large`(exit 12)になる |
| `--cwd <ディレクトリ>` | **必須** | profile の `root`(未設定なら管理ルート)。**空・未指定は `usage`(exit 2)** — レビュー経路(§5)は省略を許すが、**実装経路は向きが反転して必須**になる(§12-1) |
...
| `--log-file <パス>` | 任意 | 省略・空なら上記ログ規約の既定名で自動採番する |
...
- 診断用の `NOTE:` 行が stderr に出ることがある(§6 と同じ扱い。**`NOTE:` は失敗を意味しない**)
- **成否の判定は終了コードと `ERROR [理由コード]` 行だけで行い、stdout の内容では判定しない。** 「成果が残っているか」は stdout ではなく §12-7 の比較で判定する(スクリプトは git を呼ばないため単独では判定できない)

**注意事項**: #56 が新設するスクリプトの CLI 面は、この節を「契約の唯一の正本」として実装する。契約に載らない CLI オプションを実装しない(そうしないと、スクリプトと契約の同期義務〈冒頭ブロックの「5 点」〉が成立しない)。
