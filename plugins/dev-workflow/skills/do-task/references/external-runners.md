...
> | §9-7 一時ツリーを作れない場合の縮退 | レビュー専用 | 実装経路の縮退条件は §12-1 の承認より前の判定(parent-child・条件 A・git でない・条件 B)と、§12-2 のスナップショット取得失敗(`take` の exit 30) |
...
| `features.implementer` の**実行**可否 | セッション内の判断 | 名前が安全でも**実行そのものはセッション初回に明示承認を得るまで保留**し、承認が無い間は内蔵 implementer で実行する。**承認・拒否はどちらもセッション内で 1 回の判断として保持**し、**取り消しは「保持中の判断を破棄して未判断に戻す(次の委託時に再度承認を求める)」ことを意味する**。**解決後の起動内容、または `secret_paths` にマッチするファイル集合が変わったら承認を取り直す**。**無人(`--unattended`)では承認を求めず、外部に解決しない**(内蔵で実行する。§12-1。決定録 2026-09-23 の決定 8) |
...
2. **自ホストと別 CLI か**(共有。design §5-5): ホスト自身と同じ CLI をランナーとして起動しない(二重課金の回避)。ホスト判定は環境変数の**ベストエフォート** — `DEV_WORKFLOW_HOST_CLI`(明示上書き)→ `CLAUDECODE`(Claude Code。実測確認済み)→ `CODEX_SANDBOX` / `CURSOR_AGENT`(**未検証**)。判定できないホストでは素通りするので、疑わしいときは `DEV_WORKFLOW_HOST_CLI` を設定する。この環境変数の列は `loop.sh`(ship-task の無人ループ)もホストのセッションの中の判定に使い、`loop-selftest.sh` がこの列との一致を照合する
...
- **既定表に無いランナーを `--command` で使う場合**(レビュー経路): 任意コマンド実行に等しいので、`--dry-run` の出力(解決後コマンド)をユーザーに提示し、**明示承認を得てから**本実行する(`secret_paths` の有無に関わらず)。profile に書かれたコマンドを使ってはならない(§1 の信頼モデル)。**無人(`--unattended`)では明示承認を得られないので起動せず**、内蔵編成で続行して報告する([../../ship-task/references/unattended-mode.md](../../ship-task/references/unattended-mode.md) の D15)
...
3. **`secret_paths` にマッチするファイルが実ツリーに存在するプロジェクトでは、profile の宣言だけで自動起動しない。**(**レビュー専用**。実装経路は決定 8 により読み替える — 一時ツリーへの絞り込みが無いため「渡す対象(パス一覧)を提示」が成立せず、代わりに「作業ツリーごと外部に見える」ことを明示して承認を取る。§12-4) 渡す対象(パス一覧)とランナー名を提示し、**明示確認を取るまで停止する**。無人(`--unattended`)では確認を取れないので起動せず、内蔵編成で続行して報告する(unattended-mode.md の D15)
...
7. **一時ツリーを作れない場合**(worktree を作れない・非 git リポジトリ)は、外部 CLI がリポジトリを直接読めるため**指示だけでは機密の読み取りを防げない**。**この場合はレビュー経路の外部ランナーを起動しない**。これは意図した縮退であり、理由を報告する(design §5-9 の縮退理由「一時ツリーを作れない」)。**実装経路はそもそも一時ツリーを使わないため、この判断は適用されない**(実装経路自身の縮退条件は §12-1 の承認より前の判定〈parent-child・条件 A・git でない・条件 B〉と、§12-2 のスナップショット取得失敗〈`take` の exit 30〉による)
...
- **無人(`--unattended`)では外部に解決しない**。内蔵 implementer で実装し、理由を報告する(承認はセッション初回の明示承認で、無人では得られない。unattended-mode.md の D5)
...
- **承認より前に判定する縮退(#81)**: parent-child のほかに、次の 3 つでも外部委託を行わず、**承認を求めずに**内蔵 implementer へ縮退して理由を報告する。**判定は do-task の Phase 3 に入るたび(ITER ごと)に、外部の手順の手順 1 で行う**(判定の場所はそこだけ。Phase 0 の結果として読まない)。当てる順は §12-5 の段階の並び
  - **条件 A: タスク MD がファイルでない**(#81 決定 2): do-task の Phase 0 の手順 1 で保持したタスク MD が、読める通常ファイルでない(課題管理システムの本文など)。**本文を書き出した写しを `--task-md` に渡さない**(写しを渡すと `take` は通るが、本物の完了条件は照合されない)。ファイルでないものが `--task-md` に渡ったときに `take` が exit 30 `[taskmd]` で内蔵へ縮退する挙動(§12-2 の縮退 ⑤)も契約とする
  - **git でない**(#81 決定 3): 対象が git リポジトリでない(Phase 0 と同じ判定。base-commit.md)。**事前検査(`--precheck`)を打たずに**内蔵へ縮退する(`take` の `not-git` と同じ結末を、承認より前に決める。`--precheck` は git でない対象では rc 2 で止まる)
  - **条件 B: そのセッションで最後に承認した一覧に filter がある**(#81 決定 3)
    ...
    - 材料は、`diff-snapshot.sh --cwd <管理ルート> --precheck [--accept <最後に承認した値>]` を打ち直した結果にする。`--precheck` は内容を変換するプログラムを起動しない(実測)。stdout の受け方は base-commit.md の事前検査と同じ(`--accept` を付けないときは直接実行して終了コードだけを見る。付けるときはリポジトリの外の一時ファイル・rc を先に見る・読んだら消す)

      | 結果 | 扱い |
      |---|---|
      | rc 0 で、stdout に `GIT_CONFIG_COUNT` が出た | 内蔵に縮退 |
      | rc 0 で、出なかった | 外部の手順へ進む |
      | rc 22 | Phase 0 と同じく人の承認を仰ぐ。承認されたら、その値を最後に承認した値として打ち直す。承認されなければ停止する(Phase 0 と同じ) |
      | それ以外 | 終了コードと stderr を報告して停止する(Phase 0 と同じ閉じた規則) |

    - **理由**: `implement-guard.sh` は filter の無効化(`--precheck` が stdout に出す環境変数)を受け取らないので、承認済みの filter がある状態で `take` と `compare` の git を打つと、その clean フィルタをホスト側(サンドボックスの外)で起動しうる(§12-2 の受け入れた限界 (c) の R1)
...
**受け入れた限界(`implement-guard.sh` が塞がないもの。別途扱う)**:
...
- (c)**起動後の git がリポジトリ設定のプログラムを実行する経路の残り**(#81 決定 3 で実測に合わせて書き直した): 外部が足したもののうち、新しい filter・新しい `include.path` / `includeIf.*`・include 先に足された検査対象のキー(`filter.*` 等)・`filter` 属性(`lfs` を除く。R2)の適用先の追加は、共通工程の ① `--precheck` が exit 22 で止める(`working-tree-encoding`・`ident` 属性は、承認できない疑いとして止める。`.git/config`・`config.worktree` の生バイトの変化は、`compare` の hooks と config の検査でも止まる)。`.gitattributes` に `diff=` などほかの属性を足しても `--precheck` を通るが、`compare` は `--no-textconv` で取るので textconv を起動しない(`.git/info/attributes` は、`filter=`・`diff=`・`-diff`・`binary`・`working-tree-encoding=` を含む行があると、ファイル全体が承認できる疑いとして exit 22 で止まる)。残る経路は次のとおり
...
- (f)**使い捨ての index に対する `write-tree` は、到達不能な tree オブジェクトを `.git/objects` に残す**(index・HEAD・refs・stash・config は変えない)。加えて、**refs と stash は linked worktree の間で共有される**ので、背景実行中に別の worktree(別セッション)で行われた作業が git メタの変化として混ざりうる(§12-4 の限界③ と同じく、比較はプロセスの主体を区別しない。`CHANGE=refs` の詳細に ref 名を出すので、人が見分ける)
...
**注意事項**: **機構に頼れない**(公式文書は既定の workspace-write でも `.git` を読み取り専用に保つと書くが、0.153.4 では未実測で、実効サンドボックスは設定で上書きされうる — §12-4 の限界⑧。`.git` の保護を指定する設定も無い — `sandbox_workspace_write` の項目は `exclude_slash_tmp` / `exclude_tmpdir_env_var` / `network_access` / `writable_roots` の 4 つのみ。2026-09-17 確認)ため**検出に倒す**。design §5-20(git 出口の規律は ship-task のみ)は内蔵サブエージェント前提であり、外部委託で初めて「規約の外にいる実行主体」が生まれる。
...
8. **実効サンドボックスモードは argv とヘルプだけでは確定できない**(決定 38。公式文書と実測に合わせて書き直した — #81 決定 4。出典・確認日は design §7-3)。判定 3′ はフラグの実在しか見ず、config / permission profile / managed config で上書きされうる。**§12-2 の「外部の書き込み範囲外」という保証は「書き込み範囲が実際に限定されていること」を前提にするため、この限界が前提そのものに掛かる**
   - **設定の優先順位**(codex の公式文書。強い順): CLI フラグ・`-c` → プロジェクトの `.codex/config.toml`(信頼済みのときだけ)→ profile → ユーザー設定 → cloud-managed → system → built-in defaults。`--sandbox workspace-write` は `sandbox_mode` より強い
   - **CLI では固定されないキー**: 例は `sandbox_workspace_write.writable_roots`・`network_access`。permission profile(`default_permissions`・`[permissions.<name>]`。Beta)は、旧来の sandbox 設定(`sandbox_mode`・`[sandbox_workspace_write]`)と併用しない(公式の config-reference)。公式の Permissions ページによれば、読み込んだ設定ファイルのどれかに `sandbox_mode` がある・`--sandbox` を渡した・選んだ config profile が `sandbox_mode` を持つ、のどれかなら、`default_permissions` ではなく旧来の sandbox 設定を使う。例外は managed の `allowed_permission_profiles` で、permission profile を使わせる。したがって `--sandbox workspace-write` を渡しても、実効の書き込み範囲は、旧来の sandbox 設定では CLI で固定されないキーで変わりうる。managed の `allowed_permission_profiles` があれば、`--sandbox` を渡しても permission profile が使われる。どの profile か(`default_permissions`)とその中身は、許可リストの範囲で通常の設定の層(信頼済みならプロジェクトの `.codex/config.toml` を含む)で決まるので、それでも変わりうる(Permissions ページ: profile も通常の設定の層の仕組みに従い、優先度の高い層が同じ名前の profile の項目を足したり置き換えたりできる)。どれも未実測
   - 実測: ローカルの `~/.codex/config.toml` は、2026-09-17 に `danger-full-access`、2026-09-23 に `sandbox_mode = "workspace-write"` だった
   - **`.git`・`.codex` の保護**(公式の Agent approvals & security の「Protected paths in writable roots」。0.153.4 では未実測): 既定の workspace-write でも、`<writable_root>/.git` はディレクトリでもファイルでも読み取り専用で、`.git` がポインタファイル(`gitdir: …`)なら解決先も読み取り専用。`.agents` と `.codex` は、ディレクトリとして在るときに読み取り専用(再帰的)。**起動前に `.codex` がディレクトリとして無ければ、この保護の条件に当たらず、走行 N が `.codex/` を作れる**。その場合、走行 N が書いた `.codex/` が差し戻しの走行 N+1 の書き込み範囲を広げうる(未実測)
   - **`compare` への出方は、置かれ方で変わる**: 追跡済みなら、ignore の規則に当たっても追跡差分(`tracked-diff`・`status`)に出る。未追跡かつ非 ignore なら、新規は `untracked-added`、起動前から在るファイルの書き換えは `untracked-changed`。未追跡かつ ignore 済みなら収集対象外で、どの要素にも出ない(上の限界 ②⑤)

### 12-5. 報告

**段階の並び**(**無人判定(§12-1。決定録 2026-09-23 の決定 8)を先頭に置き**、parent-child 判定を次にして無駄な承認要求を避ける。決定 16・4・29 の順序を整理したもの。承認より前の「タスク MD がファイルでない」「git でない」「承認済み filter」は #81 決定 2・3 で足した。判定は §12-1):

```
無人判定 → parent-child 判定 → タスク MD がファイルでない → git でない → 承認済み filter → 承認 → スナップショット取得(非 git は不可)→ 存在 → 自ホスト → モード確立 → 疎通
```
...
- **比較不能時の報告テンプレ(決定 48)**: 決定 44 の「比較不能」は縮退でも引き継ぎでもない第 3 の結末なので、上の 2 つとは別のテンプレを持つ。必須要素は ①**何が一致しなかったか**(マニフェスト全体 / スナップショット本体 / タスク MD の照合・復元)②**そのために判定できなくなった要素**(①②③ の作業ツリー比較は「不明」、④⑤ の git メタは取得できていれば値を出す)③**以後の自動処理を停止したこと**、および**その時点までに実ツリーへ何をしたか** — 復元を試みたか / 試みたなら**削除とコピーのどちらまで成否が確定しているか** / **現在のタスク MD の状態**(元のまま・削除済み・復元済み)。**「自動復元を行っていない」と一律に報告してはならない**(決定 49。復元は「削除してから `cp -a`」なので、**コピーが失敗した時点で実ツリーは既に変わっている**)④**保護領域とログのパス**⑤**ユーザーに求める判断**(退避コピーを信頼して復元するか / 実ツリーの現状を正として進めるか / 外部の成果を破棄するか)⑥**再開の条件**(判断が示された後に、どの手順から続けるか)。**判断待ちの間は保護領域を削除しない**(決定 37 の「正常終了時は削除」は比較不能には掛からない)
  ...
  - **再開の条件(必須要素 ⑥)のうち、① に `GIT_CONFIG_COUNT` が出て ② を打たなかった比較不能**(#81 決定 3。`hooks-changed` / `config-changed` と同じく、元へ戻してから打ち直す形):
    1. 人が、外部が足した設定・属性・hooks を、**git を打たずに**ファイルを読んで元へ戻す。何が足されたかは、手順 5 の ① の一覧と NOTE(`NOTE: 無効化して実行:` の `core.fsmonitor` / `core.hooksPath` と、`NOTE: promisor 構成:` の `extensions.partialclone` / `remote.<名>.promisor` を含む)、`.git/config`・include 先・`.gitattributes`・hooks のファイルで見る
    2. 手順 5 を ① から打ち直す
       - rc 22 なら、出た一覧を人が見て承認するかを決める(承認した値が最後に承認した値になる)
       - stderr の `NOTE: 無効化して実行:` か `NOTE: promisor 構成:` で始まる行(すべて)が、起動前の報告に出した手順 1 の `--precheck` の同じ行と一致することを確かめる。違えば 1 に戻る(承認ダイジェストは `core.fsmonitor` / `core.hooksPath` などを含まず、② の config の検査も include 先を見ない〈`.git/config`・`config.worktree` の生バイトだけ〉。戻し忘れた `core.fsmonitor` は、この照合でしか分からない。`NOTE: 置換参照を無効化して実行` は照合しない — `refs/replace/*` は 1 の戻す対象〈設定・属性・hooks〉に無く、その変化は ② の `CHANGE=refs` に出る)
