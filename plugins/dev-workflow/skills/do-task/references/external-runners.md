...
**段階の並び**(parent-child 判定を先にして無駄な承認要求を避ける。決定 16・4・29 の順序を整理したもの。承認より前の「タスク MD がファイルでない」「git でない」「承認済み filter」は #81 決定 2・3 で足した。判定は §12-1):

```
parent-child 判定 → タスク MD がファイルでない → git でない → 承認済み filter → 承認 → スナップショット取得(非 git は不可)→ 存在 → 自ホスト → モード確立 → 疎通
```
