...
5. **共通工程(外部 CLI を起動した後は成功・失敗を問わず必ず通す。分岐より前に置く)**: **まず保持したダイジェストをマニフェストとスナップショット本体に照合し、一致した場合のみそれらを基準として信頼する**(§12-2。**照合を比較より後に置くと、外部に改竄された基準で「変化なし」と判定し、照合のある引き継ぎ工程自体を通らない**)。**突合はファイル単位**で行い、**不一致・計算不能は当該エントリのみ報告してスキップする**(全体中止にしない)。そのうえで 5 要素を再取得して起動前と比較する。**モード確立・疎通の判定で落ちた経路も外部 CLI を起動済みなのでここを通る**。**git メタ(HEAD / index / stash)に変化があれば必ず報告**し、作業ツリーの比較で外部が書いた分を識別して diff を提示する(§12-3。この工程を分岐の後に置くと、検出策が守るべきケースそのものを素通りさせる)
6. **分岐**(発火条件は §12-7):
...
7. **引き継ぎの手順**: ①**外部ランナーのログファイルのパスを内蔵 implementer へ必ず渡す**(何をどこまでやって、なぜ止まったかは diff より生出力に出る)②ダイジェストの照合は手順 5 で済んでいる③**外部が更新したチェックボックスを信用せず**、**§12-2 の退避物の実体照合(種別・モード・生バイトハッシュ・symlink のリンク文字列)と復元先パスの各階層の symlink 検査を通ったエントリに限り**、実ツリー側の同名エントリを削除してから退避コピーで復元する(**ダイジェストは退避コピーの中身には掛からず、復元先の経路も見ないため、この 2 つを省くと改竄された退避物での上書きやリンク先の無関係なファイルの削除が起きる** — 手順とスキップ条件は §12-2 が正本)。**失敗・スキップしたエントリを列挙して報告**する。外部が改変・削除した他の未追跡ファイルも退避コピーを基準に識別して提示する(未追跡ファイルは `git diff` に出ないため退避コピーが唯一の復元元)④タイムアウトの kill は**書き込み途中のファイルを残しうる**
