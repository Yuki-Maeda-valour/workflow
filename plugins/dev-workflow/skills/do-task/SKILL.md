...
**implementer**(モデルは profile の `implementer_model` に従う。単一枠の能力帯は軸 3)を起動する。**`name` は `implementer`**(**内蔵で実行するフル段階に限り**、以降の差し戻し・STATUS 問い合わせの宛先になる。**外部 implementer は宛先にならない** — 軸 5)。委託プロンプトに必ず含めるもの:
...
実装の委託先は Phase 0 の解決結果に従う。内蔵に解決されたときはここまでで起動し、Phase 4 へ進む。
...
5. **共通工程(外部 CLI を起動した後は成功・失敗を問わず必ず通す。分岐より前に置く)**: 5 要素を再取得して起動前と比較する。**モード確立・疎通の判定で落ちた経路も外部 CLI を起動済みなのでここを通る**。**git メタ(HEAD / index / stash)に変化があれば必ず報告**し、作業ツリーの比較で外部が書いた分を識別して diff を提示する(§12-3。この工程を分岐の後に置くと、検出策が守るべきケースそのものを素通りさせる)
6. **分岐**(発火条件は §12-7):
...
7. **引き継ぎの手順**: ①**外部ランナーのログファイルのパスを内蔵 implementer へ必ず渡す**(何をどこまでやって、なぜ止まったかは diff より生出力に出る)②**復元・比較の前に、保持したダイジェストとマニフェスト・スナップショットを照合**し、一致した場合のみ信頼する。**突合はファイル単位**で行い、**不一致は当該エントリの復元を中止して報告、計算不能は「突合不能」としてスキップして報告**する(全体中止にしない)③**外部が更新したチェックボックスを信用せず**、**実ツリー側の同名エントリを削除してから退避コピーで復元**し、**失敗・スキップしたエントリを列挙して報告**する。外部が改変・削除した他の未追跡ファイルも退避コピーを基準に識別して提示する(未追跡ファイルは `git diff` に出ないため退避コピーが唯一の復元元)④タイムアウトの kill は**書き込み途中のファイルを残しうる**
