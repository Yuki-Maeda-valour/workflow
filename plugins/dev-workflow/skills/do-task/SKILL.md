...
3. **起動前のスナップショットと退避**: §12-2 に従い 5 要素を記録し、未追跡かつ非 ignore のファイル全体を保護領域へ退避する。**マニフェストとスナップショット本体のダイジェストをセッション文脈に保持**し(外部が書き換えられない唯一の領域)、**保護領域のパスを報告に出す**。**clean は要求しない**(dirty のまま委託する)。**全体を縮退するのは §12-2 の 4 つ**(非 git / index がコンフリクト中 / 保護領域を外部の書き込み範囲外に解決できない / 退避が上限を超えた)**だけ**で、該当したら外部委託を行わず内蔵へ縮退して報告する。**読めないエントリ(壊れた symlink 等)は当該エントリのみ記録して続行し、そのパスが検出の死角になったことを報告する**
4. **起動**: 上の共通項目に加えて、委託プロンプトで **`commit` / `stash` / `branch` / `reset` / `push` を禁止**して起動する(§12-3。機構では防げないため、禁止と実行後の検証の 2 段で対処する)
5. **共通工程(外部 CLI を起動した後は成功・失敗を問わず必ず通す。分岐より前に置く)**: **まず保持したダイジェストをマニフェストとスナップショット本体に照合し、一致した場合のみそれらを基準として信頼する**(§12-2。**照合を比較より後に置くと、外部に改竄された基準で「変化なし」と判定し、照合のある引き継ぎ工程自体を通らない**)。**一致しなければ「比較不能」**として扱い、**①②③ の比較を「変化なし」と判定せず**(変化の有無は「不明」と報告)、**その基準での復元を一切行わず**、手順 6 の比較不能の分岐へ進む(§12-2・§12-7。**この照合はマニフェスト全体に掛かるので、不一致から「どのエントリが」は特定できず、ファイル単位のスキップでは処理できない**)。**一致した場合に限り** 5 要素を再取得して起動前と比較する。**モード確立・疎通の判定で落ちた経路も外部 CLI を起動済みなのでここを通る**。**git メタ(HEAD / index / stash)に変化があれば必ず報告**し、作業ツリーの比較で外部が書いた分を識別して diff を提示する(§12-3。この工程を分岐の後に置くと、検出策が守るべきケースそのものを素通りさせる)
6. **分岐**(発火条件は §12-7):
...
7. **引き継ぎの手順**: ①**外部ランナーのログファイルのパスを内蔵 implementer へ必ず渡す**(何をどこまでやって、なぜ止まったかは diff より生出力に出る)②ダイジェストの照合は手順 5 で済んでいる③**外部が更新したチェックボックスを信用せず、タスク MD だけを退避コピーから復元する**(§12-2 の決定 45。**他の未追跡ファイルは復元しない** — 一律に復元すると手順 6 の「**巻き戻さず引き継ぐ**」と衝突し、外部が途中まで書いた実装成果を消す)。**復元は §12-2 の退避物の実体照合(種別・モード・生バイトハッシュ・symlink のリンク文字列)と復元先パスの各階層の symlink 検査を通った場合に限り**行い、実ツリー側の同名エントリを削除してから `cp -a` する(**ダイジェストは退避コピーの中身に掛からず、復元先の経路も見ないため、この 2 つを省くと改竄された退避物での上書きやリンク先の無関係なファイルの削除が起きる** — 手順とスキップ条件は §12-2 が正本)。**失敗・スキップしたエントリを列挙して報告**する。**外部が改変・削除した他の未追跡ファイルは、退避コピーを基準に差分を識別して提示するだけに留める**(未追跡ファイルは `git diff` に出ないため退避コピーが唯一の基準。復元するかは内蔵 implementer とユーザーが判断する)④タイムアウトの kill は**書き込み途中のファイルを残しうる**
8. **報告(サイレント縮退禁止)**: どちらの担い手で実装したか・承認の有無・起動前に縮退したか走行中に引き継いだか・残っていた変更・保護領域とログのパスを必ず報告する(テンプレは §12-5)。正常終了時は完了後に保護領域を削除し、引き継ぎ時は報告後まで残してパスを提示する(§12-2)
