...
#     起動後の再取得と比較・引き継ぎの判定は **implement-guard.sh が行う**(前後に起動し diff を提示するのは呼び出し側)
...
#         プロセスの傍らで行われることになる。
#
# 副作用: ログファイルの書き出しと、一時領域(保護領域)の作成・削除のみ。
#         **ヘルプ照合・プローブ・本実行の出力を受ける scratch は「保護領域」に置く**
#         (§12-2 の解決順 $XDG_STATE_HOME → $HOME/.local/state → $HOME/.cache。決定 40)。
#         **プローブの cwd は一時領域ではなく本実行と同じ --cwd**(決定 43・§12-1)。
