...
Explore(読み取り専用)エージェントを**観点グループ別に単一メッセージで並列起動**する(design §5-13。各 Agent には下表の `name` 列を付ける。実行段階の判定と縮退は §5-17 に従う — フル段階では追加調査を同じ name へ `SendMessage` で依頼し、最小段階では直列セルフスキャンへ縮退する):
