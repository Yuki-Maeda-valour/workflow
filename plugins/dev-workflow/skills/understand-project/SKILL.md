...
1. **読み取り専用(キャッシュを除く)**。コード・ドキュメント・設定を一切変更しない。編集のツール(Edit・NotebookEdit)は使わない(frontmatter では禁じない — 呼んだ後のターンの残りにも効き、呼び出し側の skill の編集まで止めるため。design §7-3)。唯一の書き込みは把握キャッシュ `.claude/grasp.md` のみ(design §5-18 の規律に従う)
...
5. **時間対効果と段階的深化**。`--quick` は初期把握に留め、実作業では対象領域の現在の一次情報を必ず読む。全リポジトリ通読は不要で、精度が必要な後続 skill(/create-task 等)が対象領域を深掘りする
...
- **参照だけに使う**: `.claude/grasp.md` は前回の要約・関連パス・未確認事項を知る索引であり、HEAD 一致・過去の把握レベル・dirty 状態を理由に早期終了しない。毎回 Phase 0 と、要求に応じた現在の対象領域・依存先を読む
- **累積しない**: 過去の把握レベルを累積して今回の読取り範囲を決めない。今回読んだ範囲・日時・未確認事項を独立して記録する
- **実行末尾で更新**: Phase 5 のサマリー+メタ(今回読んだ範囲 / 日時 / 未確認事項)を `.claude/grasp.md` に Write する(gitignore 対象)
