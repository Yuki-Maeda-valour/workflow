...
/understand-project ──→ /create-task ──→ /do-task ──→ /update-doc --task ──→ PR
 (把握。grasp は参照索引、  (種別判定+設計書生成。   (implementer 委託+機械検証   (完了タスク駆動の差分同期。
  毎セッション hook が促し)   --refactor = リファクタ分析)  +実動確認+独立レビュー)     タグ昇格・ADR・図・索引)
                             └──────────────── /ship-task(一気通貫)────────────────┘
                              ...
                              停止条件(要件不明の残存・品質ゲート赤・スコープ縮小・レビュー未収束・
                              実動確認の結果待ち など)に当たったときだけ停止する
...
無人運用(任意・既定オフ): loop.sh = 人のシェル / cron から起動する同梱スクリプト(skill ではない)。
                    明示した 進行中_ を 1 周 = 新しいヘッドレスセッションで /ship-task(無人モード)に通す
```
...
### 無人ループ(loop.sh。[決定録 2026-09-23](minutes/2026-09-23_壁打ち_ループエンジニアリング.md))

外側の while を skill にせず、同梱スクリプト `loop.sh` が回す(while を skill にすると 1 つのセッションの中で回り、「1 周 = 新しい文脈」が崩れる)。skill の数と責務は変えない。ship-task の入口 `--task=<タスク MD>` と無人モード `--unattended`・完了条件の改竄ガードは #67 で実装した(無人モードの正本は [`ship-task/references/unattended-mode.md`](../plugins/dev-workflow/skills/ship-task/references/unattended-mode.md))。**残りは後続**(`loop.sh`・ブレーカー・実 CLI の実動確認 = #68〈#81 の後〉/ 発見ループ = #69)。
...
- **1 周の中身**: ホスト CLI をヘッドレスで起動し、`/ship-task --task=<タスク MD> --unattended` を走らせる(Phase 1 を飛ばし、Phase 2 の続行判定から do-task → update-doc → PR)。周はブランチの無い worktree(detached HEAD)で始まるので、ship-task は **Phase 2 より前に**作業ブランチ `task/{名}` を作る。git の出口は ship-task のまま(§5-20)。**merge はしない**
- **拾う対象と順番**: タスク MD に無人実行のメタ行 `> **無人実行**: 可`(書式・照合パターンは [`create-task/references/task-template.md`](../plugins/dev-workflow/skills/create-task/references/task-template.md) の記法の規約)がある `進行中_` だけ。`> **作成日**` の古い順(同じ日はファイル名順)。次のどちらかがあるタスクは拾わない(専用の状態ファイルは持たず、git の状態だけで判定する): 作業ブランチ `task/{名}`(ローカルかリモート。リモートは読み取りだけで確かめる。PR 待ち・保留中を含む)/ 前の周が残した worktree(落ちた周を含む)
- **止まったとき**: ship-task の無人モードは、止まる理由を原因で 2 つに分ける(対話点ごとの扱いは unattended-mode.md の表)
  - **保留**: そのタスクに固有の、人の判断が要る停止(Phase 2 の停止・`完了_` への改名前の報告点〈§5-10〉と許可の要る操作の拒否・基準不明など)。`保留_` に改名し、理由(どの停止条件か・人が次にすること)を定型の保留の行(1 行)で追加修正記録に書き、**作業ブランチに commit する**(停止時はタスク MD を stage しない、という対話時の契約を読み替える)。保留の行は「記録あり」に数えない(do-task の新規着手の判定と再開判定を崩さない)
  - **失敗扱い**: 環境・リポジトリの状態・ツールの異常、周の中の照合(改竄ガード)に通らない、無人の前提を欠く。**`完了_` への改名後の停止は失敗扱い**(update-doc の報告点・未承認、許可の拒否、push・PR の失敗。完了した実装を `保留_` と呼び直さず、未承認の doc を commit しない。§5-10 の例外)。改名・保留の行・commit をせず、worktree を残す(§5-1 の例外)
  - 周の結末は `PR` / `縮退` / `保留` / `失敗扱い` の 4 つで、ship-task は完了報告の最後に結末の行を書く。周の時間上限は `loop.sh` が打ち切るだけで、時間切れの周と許可待ちで固まった周は、下の「隔離」のとおり worktree を残す。`loop.sh` は改名しない
- **隔離**: 周ごとにローカルのデフォルトブランチから使い捨ての worktree を作り(fetch はしない。リモートの作業ブランチの有無は読み取りだけで確かめる)、その中で走らせる(人のチェックアウトに触れない)。周が正常に終わった(PR を開いたか、`保留_` を commit した)ときだけ worktree を消す。落ちた・時間切れ・未 commit が残った周は worktree を消さずに残し、そのタスクは以後拾わない。ループは次のタスクへ進み、連続する失敗はブレーカー(§4)で止める。残った worktree は朝の報告に並べる。**タスク MD はローカルのデフォルトブランチに commit 済み(task_dir が git の追跡下)であることが前提**で、追跡外なら `loop.sh` は起動時に止まる
- **権限**: 許可リストで最小化して走らせ、**全許可のモードは使わない**(Claude Code の全許可フラグは、インターネット接続の無いサンドボックスでだけ推奨される。§7-3)。許可リストはホストの利用者設定か `loop.sh` の起動引数にだけ置き、profile などリポジトリが書ける場所には置かない。**リポジトリ側のホスト設定(プロジェクトの許可規則・hook・MCP 宣言など)は読み込ませない起動を既定にする** — ヘッドレスではホストの承認ゲートが働かず(§7-6)、リポジトリを書ける者が無人実行の権限を広げられるため。読ませない手段が無いホストでは無人ループを起動しない。**外部 implementer は使わない**(承認はセッション初回の明示承認で、無人では得られない。内蔵 implementer で走る)
- **起動事実**: ホストごとのヘッドレス起動の事実は §7-3、実際に使うコマンドの雛形は `loop.sh` の既定表(ホスト名 → 雛形)に置く。雛形の上書きは `loop.sh` の起動引数だけで受け付け、profile に書かれていても無視して報告する。skill 本文と解決表には書かず、役割語も増やさない(起動は委託ではない)。ホストのセッションの中から起動されたと判定したら `loop.sh` は止まる(§5-5。判定はベストエフォート)
- **発見ループ**: 人の採用を飛ばして `進行中_` に積む領域は設けない。発見ループは `候補_`(§5-1)を積むだけで、採用は人が /create-task に通す
...
#     この項目が**無い**場合、または値が空(null・空文字)の場合のフォールバック(既定 serena)は従来どおり
#     (空以外は型を問わず「値がある」として扱い、3 値でなければ停止する)。旧値名は 2026-09-22 に廃止した
#     (出典は §7-4 末尾の決定録)
...
features:
  ...
  implementer: internal      # internal(既定)| 既定表(external-runners.md §12-6)にある実装用ランナー名。
                             # 既定表外の名前は profile から受け付けない(無視して報告。runners と同じ
                             # 信頼モデル・§7-2)。宣言されていてもセッション初回に明示承認を得るまでは
                             # 内蔵で実行する(承認・拒否の保持、取り消しの意味、取り直しが要る条件は
                             # external-runners.md §1 の信頼モデル表が正本。ここには再掲しない)
                             # ※ 旧値 cursor は既定表の名前ではない(既定表の名は cursor-agent)。既定表外の
                             #   名前として扱われる。実装用には「既定表外を引数で明示指定する」経路を
                             #   設けない(runners と非対称)。多様性が目的なら runners へ移す
  ...
  # ── 無人ループ(loop.sh の調整値。§2。有効化のスイッチではない)──
  loop:                      # 全項目フォールバック: キーが無い・値が無い項目は loop.sh の既定値を使う
    iteration_timeout: 3600  # 例。1 周の壁時計上限(秒)。超えた周は loop.sh が打ち切り、worktree を残して以後拾わない(§2)
                             # 既定値と不正な値(数値でない・0・負)の扱いは #68 で決める
                             # ブレーカー(最大周回数・連続失敗・時間予算〈ループ全体〉・停止ファイル)の項目は #68 で足す
  # ※ 無人ループの起動の雛形の上書きは profile では受け付けない(loop.sh の起動引数だけ。書かれていても無視して報告する)
  # ※ 無人ループの許可リストも profile では受け付けない(ホストの利用者設定か loop.sh の起動引数だけ)。信頼モデル
...
1. **タスク保存先と命名**: 保存先は [`create-task/references/task-directory.md`](../plugins/dev-workflow/skills/create-task/references/task-directory.md) を正本として、管理プロジェクトルート基準で profile の `task_dir` → 既存状態名 MD の検出 → `docs/tasks` の順に解決する。`root` は本体ソース用で保存先の基準にはしない。ファイル名は `進行中_{タスク名}.md` → 完了時に同じディレクトリで `完了_{タスク名}.md`。中断は `中断_`、保留は `保留_`。無人ループ(§2)で ship-task が自分で止まったタスクは、ship-task の無人モードが `保留_` に改名し、理由を追加修正記録に書いて作業ブランチに commit する(時間切れ・許可待ちで固まった周は改名せず、worktree を残す。§2。**例外**: 「失敗扱い」の周〈環境・ツールの異常・改竄ガード・無人の前提を欠く・Phase 0 の 3 の停止・`完了_` への改名後の停止など〉は `保留_` にせず、改名・保留の行・commit をしないで worktree を残す — [`ship-task/references/unattended-mode.md`](../plugins/dev-workflow/skills/ship-task/references/unattended-mode.md))。発見ループが積む採用前の候補は `候補_`(正本・helper への追加は発見ループの実装時。[決定録 2026-09-23](minutes/2026-09-23_壁打ち_ループエンジニアリング.md))
...
5. **ホスト自身と同じ CLI を Bash から起動しない**(別課金・二重課金の回避)。**ホスト相対の規約**として読む — 起動を禁止する対象はホストの種類によって変わり、判定手段(実行ファイル名・判定に使う環境変数の候補と上書き手段)は [`do-task/references/external-runners.md`](../plugins/dev-workflow/skills/do-task/references/external-runners.md) §3(判定順序 2)が正本。ホスト内のサブエージェントは必ずホストのエージェント機構経由で起動する。**ホストと異なる CLI を外部ランナーとして起動することは、レビュアーとしては `--runners` または `features.runners`、implementer としては `features.implementer` で宣言されている場合に限り許す**(例: Codex / Cursor をホストとする環境から Claude CLI をレビュアーに使う。implementer の判定・保護・縮退の契約は `external-runners.md` §12)。宣言が無ければ外部 CLI を探しに行かない。**人のシェル・cron から起動する無人ループ `loop.sh`(§2)はこの規約の対象外**(この規約は、ホストのセッションの中から、ホストと同じ CLI を起動することと、宣言の無い別の CLI を起動することを禁じる)。`loop.sh` は起動時に同じ判定手段(ベストエフォート。判定できないホストは素通りする)でホストのセッションの中でないかを確かめ、中と判定したら止まる([決定録 2026-09-23](minutes/2026-09-23_壁打ち_ループエンジニアリング.md))
...
8. **チェックリスト突合の機械化**: `grep -cE '^\s*- \[(x|X)\]'` で件数突合+diff との整合確認。diff は基準コミットからの追跡差分 + index + 途中 commit + 未追跡を含む(生成は do-task Phase 4 の手順 1 の `diff-snapshot.sh`)
9. **レビューの扱い**: 多モデルレビュー — **実行環境で利用可能なモデルを実行時に確認し、能力上位から順に(利用可能な最上位を必ず含めて)能力帯の異なる 2〜3 体を選び**、単一メッセージで並列スポーンする。組み合わせを固定せず、モデルの世代交代に自動追従させる(指定可能なエイリアスが利用可能一覧にあたる。**最上位 / 上位 / 標準とエイリアスの対応は §7-3**)。profile の `features.review_models` があればそれを優先する。**既定はホスト内蔵のモデルのみで編成する**(オプトイン方式。② MCP は §7-5。有効化手段が未定のため、現時点の編成は ① と、宣言時のみの ③ で構成される)— 外部 CLI や他ベンダーのモデルを探しに行かず、ホストのエージェント機構で指定可能なモデルの上位から選んで完結させる。**外部ランナーが宣言されている場合に限り**(`--runners` 引数、または profile の `features.runners`)、ベンダー横断の多様性を同一ベンダー内の能力帯差より優先し、各社の最上位級を組み合わせる(製品名・版数をハードコードせず実行時に選ぶ)。宣言があっても外部ランナーが利用不可のとき(未導入・認証切れ・レート制限・応答なし・読み取り専用未確立・一時ツリーを作れない)は、ベンダー横断を諦めて**内蔵のみの編成に縮退**し、縮退したことと理由を報告に明記する(サイレント縮退禁止)。宣言されているのに使えない場合は**エラーとして報告**する(黙って内蔵だけで済ませない)。外部ランナーの起動手順・判定順序・機密ガードは §7 と `do-task/references/external-runners.md` に従う。モデルを選べない環境では、観点(事実整合 / セキュリティ / 規約)を分けた複数レビュアーで多様性を確保する。**規模適応**: 変更対象が少数(目安 3 件以下)で矛盾の無い軽微な実行でも、独立レビュアーを 1 体以上起動する。セルフレビューは調査・機械検証を補助できても独立レビューの代替にしない。`--no-review`、profile のレビュー省略設定は互換入力として受け付けるが無効化して理由を報告する。通常の `--light` 単独では影響範囲調査を省略できるが、`--compact` の実行では `--light` と profile の影響調査省略設定を無効化し、通常形式へ戻っても影響範囲調査を必須にする。独立レビュアーまたは解決表が使えない場合はレビュー未完了を報告し、完了承認・後続公開へ進めない。→ team-lead が各指摘を実コードで裏取りし valid / invalid / needs-user にトリアージ。指摘を盲信して自動反映しない。false positive は理由を記録
10. **レビューループのセーフティ**: **収束条件は全 reviewer の APPROVED**。コストを理由にレビュー反復を打ち切らない。同一指摘の 2 回連続残存・5 ラウンド超え・`--max-review` / `--max-iter` 到達は**停止ではなく報告点**であり、状況を報告してユーザーの判断を仰ぐ。**無人モード(§2 の無人ループ)では、報告点に達したら `保留_` にして人に返す** — 判断を仰ぐ手段が `保留_` になるだけで、未収束のまま PR を出さず、無人で続行もしない(基準は下げない)。**例外: `完了_` への改名後の停止は失敗扱い**(update-doc の報告点など。完了した実装を `保留_` と呼び直さず、未承認の doc を commit しない。§2)。周ごとの壁時計の上限を超えた周は、`loop.sh` が打ち切り、§2 のとおり worktree を残して以後拾わない([決定録 2026-09-23](minutes/2026-09-23_壁打ち_ループエンジニアリング.md))
11. **実コード優先の原則**: メモリ・ドキュメントと実コードが矛盾したら実コードを信じ、矛盾を必ず注記する。裏取りなしの推測でドキュメントを書かない
12. **機密保護**: `secret_paths` のファイルは読まない(**外部 implementer には作業ツリーごと見えるためこの保証は及ばず、明示承認で扱う**。契約は `external-runners.md` §12-4)。存在の有無だけ報告。サブエージェントのログへの混入も検査対象
13. **読み取り専用エージェントの原則**: 調査・レビューは読み取り専用の種別、編集・コマンド実行は編集可能な種別に集約する(種別名は解決表 軸 1 が正本)。**読み取りのみでも追加ツール(Web 取得等)やモデルの明示指定が要る委託は編集可能な種別を使い、読み取り専用は指示で担保する**(種別選択の決定順は解決表の軸 1、要請は軸 3・軸 9)
14. **ログ永続化**: レビュー・検証の記録は `.claude/reviews/{role}-{タスク名}-iter{N}.md` に保存(タスク文脈を持たない skill は `{skill名}-iter{N}.md`)。**別のセッション・別の worktree の工程が判定に使う結果・印は、タスク MD(追加修正記録・ヘッダ)に残す** — create-task の設計レビューの行(ship-task の Phase 2 が読む)と、do-task の基準コミット行・追加修正記録(再開判定が読む)。無人モードでは、ship-task が止まった理由を**保留の行**(1 行の定型。「記録あり」に数えない)で追加修正記録に残す。設計レビューの行の `本文` は承認時の本文ダイジェスト(算出は `create-task/scripts/task-digest.py`。定義の正本は [`create-task/references/task-template.md`](../plugins/dev-workflow/skills/create-task/references/task-template.md) の記法の規約)で、行と設計の中身を結び付ける。`.claude/reviews/` のログはキャッシュで、在れば使う(§5-18 ①)。例外として、base-commit.md の新規着手の条件 ③ は、ログが有るときだけ保守側に倒す(無いときの判定は同じ。理由は base-commit.md)
...
17. **実行環境の段階判定と縮退プロトコル**: サブエージェント関連機能は環境により使える範囲が違う。skill は**着手時にツールの実在で段階を判定し**(推測しない・ユーザーに聞かない。判定手段は解決表 軸 8 が正本)、判定結果を報告に明記する。**使えるのに下の段階で回さない**:
    - **フル**(双方向の委託が可能な段階): 委託は双方向。**起動時に `name` を付け**(`implementer` / `reviewer-strong` 等)、次を積極的に使う:
      1. **完了後の追加依頼**: 差し戻し・再レビューは**同じ name への再依頼(軸 5)**。エージェントの文脈が保たれるため、再スポーン(文脈ゼロからの再調査)より速く・ぶれない。**これがフル段階の最大の利得なので、レビュー反復では必ずこちらを使う**
      2. **走行中の STATUS 問い合わせ**: **外部 implementer(下の項)を除き**、待機目安(implementer 15 分 / reviewer 20 分。**reviewer の目安は外部ランナーの既定の最大所要時間〈プローブ + 本実行 + ヘルプ照合 + 終了猶予、約 12 分〉より長く保つ** —— 短いと正常に走っている外部レビューを二重起動する)を超えたら生存を確認し(軸 7)、状態を尋ねる(軸 6)
      3. **走行中の前提是正・中止**: 委託後に前提の誤り(対象の取り違え・スコープ解釈違い)に気づいたら完了を待たずに訂正する。不要になった委託は中止を伝える(黙って結果を捨てない)
      - 宛先が失われている(軸 7 の生存確認に無く、軸 6 の送達が失敗する)ときだけ、標準段階と同じ再スポーンにフォールバックする
      - **レビュアー同士を会話させない**。独立性が失われると多モデルレビューの多様性が意味を失う。指摘の突合は必ず team-lead が行う
    - **標準**(一方向の委託のみ可能な段階): 委託は「起動 → 最終レポート」の一方向。差し戻し・追加指示は**前回成果物のパスを含めた新規起動**で代替する。応答しない委託先への STATUS 問い合わせ(M1)は省略し、待機目安を超えたら再スポーン(M2)に直行する
...
20. **git 出口の規律**: ブランチ作成・commit・push・PR 作成を能動的に行うのは `/ship-task` のみ(`/do-task` はユーザーが `--branch` 等で明示指定したときだけブランチを作る。コミットは従来どおり求められた場合のみ。**求められていないのに `/do-task` が index に触れるのは、Phase 7 で追跡済みのタスク MD を `git mv` で改名するときのリネームの stage だけ**(未追跡のタスク MD は `mv` で改名し index に触れない。どちらも commit はしない)。**外部 implementer にはこの規律が及ばない** — 外部委託は規約の外にいる実行主体を生み、機構では git 操作を止められないため、委託プロンプトでの禁止と実行後の検証〈`external-runners.md` §12-3〉で扱う)。**マージは決して行わない**。PR は品質ゲートが緑・レビューが全員 APPROVED・実動確認が「実施済」か「実施不能」(§5-19)のときだけ開き、そうでないときはブランチと commit を残して停止する。commit メッセージ規約は `git log` から推定してプロジェクトに合わせ、実装と doc は別 commit に分ける。**無人ループの `loop.sh`(§2)は、ブランチ作成・commit・push・PR を行わない**。git への書き込みは周ごとの使い捨て worktree の作成・削除だけで、ブランチ作成・commit・push・PR は周の中の ship-task が行う(無人モードで止まったときは、保留なら保留の手順の 1 commit〈`保留_` への改名と保留の行〉だけを作り、失敗扱いなら commit しない。対話の停止で実装 commit だけ残す動作は、無人では行わない。§2)
21. **チェーン実行時の責務分界**: 複数 skill を連鎖させる skill(/ship-task)は**工程の中身を再定義しない**。順序・工程間の続行判定・出口(git)だけを持ち、各工程の手順・品質基準は元の skill に委ねる。連鎖元から呼ばれた skill は、ユーザーへの次アクション提案(チェーン提案)を出さない(呼び出し元が判断するため)。無人モードでは、/ship-task が /do-task・/update-doc に `--unattended` を透過し、子の skill は止まるときに理由を保留と失敗扱いに分けて返す(改名・commit はしない)
22. **リポジトリ自身の Issue 運用**: このリポジトリを改修する作業記録は GitHub Issue 本文・コメントを正本とし、ローカルのタスク成果物は新設しない。create-task の設計成果物と do-task の対象は指定 Issue と読み替え、未指定で複数候補なら選択を確認する。チェック状態・レビューは Issue 本文と diff で確認し、完了リネームの代わりに検証結果を Issue に記録する。update-doc はその完了記録を入力にし、ship-task はタスク MD を commit せず PR に Issue をリンクする。この読み替えはこのリポジトリだけに適用し、配布スキルの task ファイル方式は維持する
23. **独立レビューの適用範囲**: プロジェクトのコード・設定・正本文書を変更する経路に適用する。調査だけ・機械検査だけ・会話出力、派生資料を出す export-doc と決定録を作る discuss-spec には適用しない
24. **検証器(`scripts/validate.py`)の免除と可変語**:
    - **免除は明示マーカーだけ**: 禁止パターン検査の行単位の免除は、行内の `<!-- validate-allow: 理由 -->` に限る(**理由の記述が必須**)。行単位で効き、コードフェンスの内外は問わない。**効くのは禁止パターン検査とホスト CLI 語検査(§7-7-1)の 2 つ**で、委託の語(§7-7)・解決表の除外の不変条件は免除規則を持たない —— **役割語へ書き換えて消せる語には免除を設けない**という線引きで、MCP サーバ名のように**生成する設定の識別子そのもので書き換えて消せない**語があるカテゴリにだけ免除を置く(**免除を持たない側の語**はマーカーでは消えず、役割語へ書き換えて消す)。以前は「禁止 / 使わない / しない / 廃止 / ではなく」を含む行を一律に免除していたが、普通の文に「〜しない」が入っているだけで混入が素通りしていた
    - **周辺プロジェクト名は環境変数で渡す**: `WORKFLOW_PROJECT_NAMES` に名前の一覧(カンマ区切り)を渡したときだけ検査し、**未設定なら 1 語も足さない**。実行環境のディレクトリを列挙しないので、検査の結果は渡した値だけで決まり、同じリポジトリならどの PC でも再現する(固有名をこのリポジトリに書かないことを優先した決定。既定で保護が外れることは受け入れ、運用で渡す)。照合は**名前の端が英数字・`_` のときだけ境界を付ける**形で行う —— `\b` は日本語と `_` を単語文字として扱うため日本語直結を取りこぼし、逆に固定で両側へ境界を付けるとハイフンで終わる名前で境界の意味が反転して取りこぼす(§7-7-1 の `Agent` と文字集合が非対称な理由もそこにある)
...
- **description は「何をするか+いつ使うか(トリガー語句)」を日本語で 150〜500 字(目安 350)**(**150 字未満・500 字超は `scripts/validate.py` が ERROR で落とす**。上限 1024 字は別の桁の歯止め)。Claude の自動呼び出し判断の材料になるため、ユーザーが言いそうな表現(「〜して」)を含める。上限 1,024 字・description+when_to_use 合計 1,536 字
...
| 実行層 | 誰が委託を実行するか(**① ホスト内蔵 → ② MCP → ③ CLI**) | ホスト依存。**解決表(§7-5)とアダプタ(review-agent.sh / implement-agent.sh)で吸収する** |
...
- **機密ガードの原則**: 宣言は「使ってよい」であって「毎回無確認で渡してよい」ではない。`secret_paths` が実在するプロジェクトでは明示確認を取ってから起動する。**レビュー経路**は読み取り専用であり、**ユーザーの git 状態(index・stash)を書き換えてはならない**。**レビュー経路の外部 CLI は一時ツリー(作業ツリーの複製)で起動する**(実装経路の保護は一時ツリーを使わないスナップショット方式で、詳細は `external-runners.md` §12-2)。cwd を限定できないランナーは既定で無効とし、一時ツリーを作れない構成では起動せず §5-9 に従って縮退と理由を報告する。なお**契約側(`external-runners.md` §9)は上書き手段を設けない**ため、この「既定で無効」は実質「使わない」として**契約されている**(表現の差であって方針の差ではない。`--command` まで閉じたのは決定 15 の却下理由と同じ根拠による意図的な強化)。ただし**機構で閉じているのは既定表ランナーへの `--command` 拒否まで**で、既定表外の名前で渡す経路は検出せず**呼び出し側 skill の義務**として課される(同 §9)
- **なぜレビュー経路で一時ツリーを必須にするか**: cwd を限定せずに起動した外部 CLI が**リポジトリ外のパスを読んだ実測**がある(§7-3)。**他プロジェクトのパスが外部ベンダーへ渡ることを防ぐ**。ランナー側のサンドボックスは既定表の読み取り専用フラグと同様に実装を信頼するしかなく(保証範囲は `external-runners.md` §3)、ランナーごとに実効性を検証できないため既定で必須にする。却下: 「限界として明記するに留める」(実害が確認されたリスクを利用者の注意力に任せることになる)。**実装経路(implementer)は書き込みを前提に起動するため一時ツリーを使わず**、起動前後のスナップショット比較で外部が書いた分を識別する(§12-2 が詳細を持つ。同じ事実を再掲しない)
- **implementer の外部化を認める**(決定 1)。本節が以前置いていた除外規定(implementer を外部化の対象から外すという範囲設定)は明示的な検討の結果ではなかったため、方針を改めた。オプトインの宣言(`features.implementer` に既定表の実装用ランナー名を指定)を維持したまま、**宣言された環境では implementer の既定を外部にする**(スキーマは §4)。以下は決定 3・4・5 の**理由(なぜそうするか)だけ**を持ち、判定順序・終了コード・スナップショットの手順・機密ガードといった契約の詳細は再掲せず `external-runners.md` §12 を参照する(本節冒頭の原則「同じ事実を複数箇所に書かない」):
  - **なぜ作業ツリーを直接使うか**(決定 3): 依存・ビルド環境がそのまま使える状態で implementer 自身にテストを回させるため。一時ツリーを使わない代わりに、起動前後のスナップショット比較で保護する
...
| **Claude Code** のモデルの能力帯(最上位 / 上位 / 標準)とエイリアスの対応。§5-9 の多モデルレビュー編成・解決表 軸 3 の単一枠既定(「単一枠では標準の能力帯を選ぶ」)が参照する。現時点の目安: fable(最上位)+ opus(上位)+ sonnet(標準)。利用可能なエイリアス群はモデルの世代交代で変わるため固定リストとして扱わず、実行時に指定可能なものを確認する | ローカル実測・実行時確認(2026-09-10) | 5-9 |
| **`codex exec resume` は `-s/--sandbox` も `-C/--cd` も受け付けない**(`error: unexpected argument '-s' found` / 同 `'-C'`)。ヘルプにも両フラグと `workspace-write` の語が存在しない | ローカル実測(2026-09-17・codex-cli 0.153.4) | 7-2 |
| `codex --help`(トップレベル)に `-s, --sandbox` と `workspace-write` が実在する(既存の 2 段照合〈argv + ヘルプ〉の機構をそのまま流用できる根拠) | ローカル実測(2026-09-17・codex-cli 0.153.4) | 7-2 |
...
| **実効サンドボックスモードは argv とヘルプだけでは確定できない**(config / permission profile / managed config で上書きされうる)。ローカルの `~/.codex/config.toml` が `sandbox_mode = "danger-full-access"` だった。**2026-09-23 の実測では `sandbox_mode = "workspace-write"`** だった | ローカル実測(2026-09-17・codex-cli 0.153.4。2026-09-23 再確認) | 7-2 |
| **Codex** の設定の優先順位(強い順): CLI フラグ・`-c` → プロジェクトの `.codex/config.toml`(信頼済みのときだけ)→ profile → ユーザー設定 → cloud-managed → system → built-in defaults。`--sandbox workspace-write` は `sandbox_mode` より強い | [Config basics — Codex 公式ドキュメント](https://learn.chatgpt.com/docs/config-file/config-basic)(旧 URL からは 308 で転送される。2026-09-23 確認) | 7-2 |
| **Codex** の CLI(`--sandbox`)では固定されないキーがある。例: `sandbox_workspace_write.writable_roots`・`network_access`。permission profile(`default_permissions`・`[permissions.<name>]`。Beta)は、旧来の sandbox 設定(`sandbox_mode`・`[sandbox_workspace_write]`)と併用しない。読み込んだ設定ファイルのどれかに `sandbox_mode` がある・`--sandbox` を渡した・選んだ config profile が `sandbox_mode` を持つ、のどれかなら、`default_permissions` ではなく旧来の sandbox 設定を使う。例外は managed の `allowed_permission_profiles`(permission profile を使わせる)。未実測(external-runners.md §12-4 の限界 ⑧) | [Permissions — Codex 公式ドキュメント](https://learn.chatgpt.com/docs/permissions)・[Configuration reference](https://learn.chatgpt.com/docs/config-file/config-reference) の `default_permissions`(2026-09-24 確認) | 7-2 |
| **Codex** の既定の `workspace-write` でも、書き込み可能な root の下の保護パスは読み取り専用: `<writable_root>/.git` はディレクトリでもファイルでも読み取り専用で、`.git` がポインタファイル(`gitdir: …`)なら解決先も読み取り専用。`.agents` と `.codex` は、ディレクトリとして在るときに読み取り専用(再帰的)。0.153.4 では未実測(external-runners.md §12-4 の限界 ⑧) | [Agent approvals & security — Codex 公式ドキュメント](https://learn.chatgpt.com/docs/agent-approvals-security) の「Protected paths in writable roots」(2026-09-24 確認) | 7-2 |
...
| **Claude Code** のヘッドレス起動: `-p, --print` で非対話に実行して終了する(ワークスペースの信頼ダイアログは出ない。ヘルプは信頼するディレクトリでだけ使うよう注記する。`-p` では、検証に失敗した設定ファイルは黙って無視されるとヘルプに記載がある)。許可の絞り込みは `--allowedTools` / `--disallowedTools`(例 `"Bash(git *) Edit"`)と `--permission-mode`(`acceptEdits` / `auto` / `bypassPermissions` / `manual` / `dontAsk` / `plan`)。`--permission-prompts none` を付けると、`--print` で許可を求める操作は自動で拒否される。`--dangerously-skip-permissions`(全許可)と `--allow-dangerously-skip-permissions` は、ヘルプが**インターネット接続の無いサンドボックスでだけ推奨**する。読み込む設定の出所は `--setting-sources`(`user` / `project` / `local` のカンマ区切り)で選べ、`--strict-mcp-config` は `--mcp-config` 以外の MCP 設定を無視し、`--safe-mode` はカスタマイズ(CLAUDE.md・skill・プラグイン・hook・MCP サーバーなど)を読まずに起動する。`--restricted` は利用者・プロジェクト・ローカルの設定ファイルを読まず(managed settings と `--settings` は効く)、`bypassPermissions` を拒否する。ただし `--tools` で名前を挙げない限り、Bash などコードを実行するツールと WebFetch を外す。MCP サーバーは `--strict-mcp-config` を併せないと読まれ、settings・git・ツール設定ファイルへの書き込みは人か設定済みの承認ハンドラだけが承認できる(ここまでの設定まわりのフラグは、**どれも効き方は未実測**)。`--max-budget-usd` で API 呼び出しの金額に上限をかけられる(`--print` のときだけ) | ローカル実測(2026-09-23・Claude Code 2.1.280 の `--help`) | 2(無人ループ) |
| **Codex** のヘッドレス起動: `codex exec [PROMPT]`。サンドボックスは `-s, --sandbox`(`read-only` / `workspace-write` / `danger-full-access`)。`--approve-for-me` は、承認の要求を `workspace-write` サンドボックスを使う自動審査へ回す。`--dangerously-bypass-approvals-and-sandbox` は確認とサンドボックスを外す(ヘルプは、外部で隔離された環境だけを想定すると注記する)。`--ignore-rules` は利用者とプロジェクトの execpolicy の `.rules` を読まない。`--dangerously-bypass-hook-trust` は hook の信頼の確認を飛ばす。**許可リストを渡す専用のフラグは `exec --help` に無い**(設定値は `-c key=value` で上書きでき、ヘルプの例に `sandbox_permissions` がある) | ローカル実測(2026-09-23・codex-cli 0.153.4 の `exec --help`) | 2(無人ループ) |
| **Cursor CLI** のヘッドレス起動: `cursor-agent -p, --print`(ヘルプは、書き込みとシェルを含む全ツールにアクセスできると注記する)。コマンドの許可は `-f, --force`(別名 `--yolo`)、ワークスペースの信頼は `--trust`(両者は別物。既存の行を参照)。ほかに許可に関わるのは `--sandbox`(`enabled` / `disabled`)・`--auto-review`(安全なツール呼び出しをサーバー側の分類器が自動で実行し、残りは確認を求める)・`--approve-mcps`(MCP サーバーをすべて自動承認)。**許可リストを渡すフラグは `--help` に無い** | ローカル実測(2026-09-23・cursor-agent 2026.09.08 の `--help`) | 2(無人ループ) |
...
- **版**: この改訂への追従は **v4.3.0** で出す。マイナーでの旧値廃止(§4 の `source_of_truth`)は §8 の「後方互換な機能追加はマイナー」と緊張するが、影響範囲の小ささ(旧値を持つのは超小規模構成の profile のみ)を理由に承知の上で選択した(§8 の基準は変えない)。移行の注記は README と `CHANGELOG.md` に置く

出典: `docs/minutes/2026-09-09_壁打ち_AI非依存の開発基盤.md`(決定 5・6・13、派生判断 17・18)/ `docs/minutes/2026-09-21_壁打ち_権威参照ファイルの一本化.md`(決定 1〜6・8〜17 と帰結。決定 4・16 の反映先は §3、決定 8・9・17 は §4。2026-09-09 決定 5 の後半を変更)
...
- **解決の規定は解決表が正本**(解決順・profile が宣言できる範囲・既定・② の未定・報告義務・縮退・役割語の一覧)。**本節は再掲しない**
- **解決表の置き場**: [`do-task/references/delegation-map.md`](../plugins/dev-workflow/skills/do-task/references/delegation-map.md)(共有アセットとして所有 skill の配下に置き、他 skill は兄弟参照で解決する。§6)。**役割語の一覧・派生名の体系・属性軸・解決順・ホストでの解決・縮退は解決表が正本**であり、本節が自ら持つのは原則(下の「なぜそうするか」)である。**上の各項の要約併記は v4.0.0(移行完了。§7-7)で終了した** — §7-2 の原則(同じ事実を複数箇所に書かない)に戻し、規定は解決表だけが持つ。**変更の向きは一方向** — 解決表を先に直す(**役割語を増やすときは解決表 §1 に足す**。本節に一覧を生やさない)
...
```bash
V='\bAgent\b|SendMessage|ListAgents|Explore|general-purpose|fable|opus|sonnet|haiku'
for s in plugins/dev-workflow/skills/*/; do grep -oE "$V" "$s/SKILL.md"; done | wc -l          # → 73
grep -rh -oE "$V" plugins/dev-workflow/skills/*/references/ \
  --include='*.md' --exclude='delegation-map.md' --exclude='external-runners.md' | wc -l        # → 21
for s in plugins/dev-workflow/skills/*/scripts; do [ -d "$s" ] && grep -rhoE "$V" "$s"; done | wc -l   # → 0
```

**この値は §7 前文の測定コマンドとその実測値とは別物であり、比較できない**(§7 側は `.claude/` を含む広い語彙で `haiku` を持たず、`\b` 境界も付けていない)。§7 側は移行順序の目安として現行のまま据え置き、**検査と到達条件は本節の語彙と除外集合だけで判定する**。

**エイリアスの世代交代への追随**: 検査語彙のモデルエイリアスは**スナップショット**であり、世の中に新エイリアスが出たら**次の 5 箇所を同時に更新する**。① 上のコードブロックの測定コマンドの語彙 → ② `scripts/validate.py` の `_MODEL_ALIASES` → ③ 同 `FORBIDDEN_PATTERNS` の日付付きモデル ID → ④ 同 版数 → ⑤ §7-3 の「最上位 / 上位 / 標準とエイリアスの対応」の表。**この順で直す**(design を先に直し、コードを追随させる。§5 の解決表と同じ向き)。⚠ **③④ から ① ② を導出しない** — ③④ は日付付き ID・版数を検出する正規表現で `mythos` を含み、**エイリアス集合の列挙ではない**。**導出はしないが同時に更新する**。⚠ **機械化しない**: 追随のトリガーが外部事象なので、① と ② を突き合わせる検査を足しても**両方が古いまま通る**。⚠ **§5-4 の「固定リストとして扱わない」とは別の話**(あちらは実行時に指定できるエイリアス集合)
...
- **なぜ機械検査とレビュー観点の両方か** — 機械検査は測定可能な不変条件になり、段階移行と相性がよい(全 skill を対象にしたまま、未移行 skill を許容リストで除外できる)。リンク検査の拡張で同手法が効くことを実測済み。ただし語彙一致では拾えない文脈依存の結合が残るため、レビュー観点も併用する
...
- **このカテゴリに免除規則は無い**: §5-24 の免除マーカーは委託の語検査には効かない。委託の語(ホスト内蔵の委託機構名・モデルエイリアス)は**役割語へ書き換えれば必ず消せる**ので、例外を書く必要がないため。書き換えて消せない語を持つ §7-7-1 とはここが分かれる

### 7-7-1. ホスト CLI 語(委託の語とは別カテゴリ。2026-09-17 追加)

上の「委託の語」は `Agent` / `SendMessage` / `ListAgents` / `Explore` / `general-purpose` / モデルエイリアスというホスト**内蔵**の委託機構名を対象にする。**このカテゴリは、ホスト内蔵の委託機構名以外のホスト結合を広く扱う**(外部 CLI 固有の語に加えて、MCP サーバ名・ホストのツール引数名も含む)。それとは別に、implementer を外部 CLI(Codex / cursor-agent / Gemini)へ委託する経路(#51)を作るにあたり、**ランナー名・サンドボックスモード名・コマンド形・プラグイン/subagent 名**という外部 CLI 固有の語が skill 本文に混入する経路が新たに生まれたため、`scripts/validate.py` に別カテゴリ(`_HOST_CLI_WORDS` / `check_host_cli_words()`)を新設した。**既存の「委託の語」検査(`_DELEGATION_WORDS`)は変更しない**(語彙・スコープ・除外集合のいずれも 1 ビットも変えていない)。

- **対象語(限定を同じ文に置く: 「全域」ではなく検査範囲(下の「検査範囲」— skill 直下 + `references/` 配下の `*.md`、除外 2 本を除く)の実測が 0 件の語だけを入れる。**正当に具体名を持つ行を免除マーカー(§5-24)で外した後に 0 件であればよい**。§7 の測定コマンドと同じ考え方だが範囲が違う)**: `scripts/` / `templates/` を含めた全域では `cursor-agent` が **26 件**(`scripts/` 25 + `templates/` 1)・`gemini` が **6 件**(すべて `scripts/`)実在する(実測 2026-09-17)が、いずれも検査範囲外にあるため対象語として選べる。この **6 系統 12 語**はランナー名(`cursor-agent` / `gemini`)・サンドボックスモード名(`workspace-write` / `danger-full-access`)・コマンド形(`codex exec` / `codex review` / `codex mcp`)・プラグイン/subagent 名(`codex-plugin-cc` / `codex-rescue`)・MCP サーバ名(`claude-in-chrome` / `chrome-devtools`)・ホストのツール引数名(`run_in_background`)
- **検査範囲**: 各 skill の **skill 直下のファイル(画像を除く)全部** + **`references/` 配下の `*.md`**(委託の語検査と同じ判断 — skill 直下を `SKILL.md` に限定しないのは、`README.md` のようなレイアウト外のファイルに置くと到達条件をすり抜けるため。既存との差分は「`scripts/` を対象外にする」点だけ)。**`scripts/`(および `templates/`)を対象外にする前提は委託の語検査と共通だが、結論は逆**: 前提(`scripts/` はモデル名や CLI 名を `--model` 引数のように外から受け取る実行層のアダプタである。design §7-2)は共有するが、委託の語検査はこの前提から「ホスト**内蔵**の委託機構名は shell / python から呼べず呼び出しとして現れない(実測 0 件)、だから `scripts/` を対象に含めてよい」と結論し、このカテゴリは同じ前提から「`scripts/` は**外部 CLI** の名前を正当に持つ場所だから対象外にする」と逆の結論になる。`templates/` も同じ理由(CLI 名を正当に持ちうる)で対象外
- **除外ファイル**: 委託の語検査と同じ 2 本(`do-task/references/delegation-map.md` / `do-task/references/external-runners.md`)を共有する。3 本目には増やさない
- **大小の扱いは既存カテゴリと異なる**: このカテゴリは **`re.IGNORECASE` で大小を無視する**。理由は `Codex に実装を委託する` のような**大文字始まりの製品名・コマンド名が最も混入しやすい書き方**であること。既存の「委託の語」は大文字小文字を区別したままなので(§7-7 の「語彙の限界」)、**同じ検査基盤の中に大小の扱いが異なる 2 カテゴリが並存する**。実測(2026-09-17): `Codex exec` / `Cursor-agent` / `Gemini` のような大文字始まりの用例は skill ツリー全域(検査範囲 + `scripts/` + `templates/`)で 0 件のため、`IGNORECASE` にしても誤検出しない。**決定 23 は「コマンド形・サンドボックスモード名・プラグイン/subagent 名」の 3 系統だけ大小無視と書いているが、ランナー名も同じ理由(大文字始まりの製品名・コマンド名が最も混入しやすい書き方であること。決定 23 の根拠文自体が `Gemini` を例に挙げている)で含める** — 実装は 6 系統 12 語すべてに `IGNORECASE` を適用する
- **境界の扱い(単語境界を付けない)**: `_HOST_CLI_WORDS` の 12 語は**単語境界(`\b`)を付けず、部分一致にする**。単語境界を付けない理由は §7-7 の「語彙の限界」がすでに明文で否定した設計だからで、`_DELEGATION_WORDS` 側の `Agent` も `\b` はやめ、**英字だけの否定後読み・先読み(`(?<![A-Za-z])Agent(?![A-Za-z])`)** に替えた(`Agentに委託` / `委託はAgentで` を捕まえ、`Agents` / `ListAgents` / `AGENTS.md` / `specialist agent` は捕まえない)。**`Agent` は部分一致ではなく英字だけの境界を持つ**ので、以下の「部分一致を選ぶ」は `_HOST_CLI_WORDS` の側の話である——**日本語文字が `\w` に含まれるため `\b` を付けると助詞が直接続く形(`Geminiに実装を委託する` / `Codex execで委託`)を取りこぼす**(偽陰性 = 到達条件をすり抜ける危険側)一方、**誤検出は ERROR で落ちる安全側**であるため部分一致を選ぶ。実測(2026-09-17): `\b` 付きだと `Geminiに実装を委託する` / `Codex execで委託` はいずれも MISS(取りこぼす)、空白区切りの `gemini を使う` だけが HIT した。**部分一致にしたため日本語隣接の取りこぼしは無いが、`my-cursor-agent-wrapper` のようなハイフン隣接では誤検出しうる** — 誤検出は ERROR で落ちる安全側なので、出たらその語を書き換えるか除外集合に足す(§7-7 の「語彙の限界」と同じ方針)。ハイフン隣接と並ぶ副作用として、コマンド形(`codex\s+exec` 等)の `\s+` は改行も含むため、行末の `codex` と行頭の `exec` のような跨行の並びにもマッチするが、これも同じ安全側の方針(誤検出は ERROR で落ちる)でそのまま許容する。**周辺プロジェクト名の検査(§5-24)が除外集合を `[A-Za-z0-9_]` まで広げ、かつ名前ごとに境界を付け外しするのと非対称なのは、`Agent` が CamelCase の固有トークンで `_`・数字が隣接する実例を持たないのに対し、プロジェクト名は小文字の一般語を含みうるため**。なお**§5-24 の免除マーカーはこのカテゴリには効く**(§7-7 とはここが違う) —— MCP サーバ名は生成する設定の識別子そのもので、役割語へ書き換えて消せないため。**ファイルごと除外にはしない**(その先で新しく混入しても捕まらなくなる)
- **既知の限界(3 件。理由は各々同じ文に書く)**: ① **単独の `codex` は検査語に入れない** — 設定ファイルパス形 `.codex/config.toml`(実測: `SKILL.md` 21 件 + `references/` 6 件、いずれもパス形)と衝突するため。コマンド形 3 種(`codex exec` / `codex review` / `codex mcp`)で CLI としての混入は検出できる。② **`read-only` は検査語に入れない** — 「読み取り専用」の英語表記という一般語と衝突するため。サンドボックスモード名は 3 値のうち 2 値(`workspace-write` / `danger-full-access`)を検出すれば、モード指定を skill 本文に書く行為自体は検出できる。③ **単独の製品名(`Codex` / `Cursor` / `Claude Code`)は検査語に入れない** — `init-project` に正当な散文用例がある(実測: `Codex` が `SKILL.md` 4 件・`references/` 3 件、`Cursor` が `SKILL.md` 2 件)ため。コマンド形・サンドボックスモード名・プラグイン名を大小無視で検出するため、実際に委託を書こうとすれば多くの場合いずれかに触れるが、**製品名だけの散文は検出されない検出漏れを承知の上で選択している**
- **到達条件**: 「0 件」だけでは検査が働いている証明にならない(このリポジトリが過去に同型の事故を踏んでいる)。**ホスト CLI 語を 1 つ含む治具 skill を `plugins/dev-workflow/skills/` 下に一時的に置き、`python3 scripts/validate.py` が ERROR を出して exit=1 になることを 6 系統(ランナー名・サンドボックスモード名・コマンド形・プラグイン名・MCP サーバ名・ホストのツール引数名)それぞれで実測し。**免除マーカーを付けた行が ERROR にならないこと・理由の無いマーカーでは ERROR のままであること**も実測する。あわせて、うち 1 系統は大文字始まり(例 `Codex exec`)で確認する**。**加えて、助詞が直接続く形(例 `Codex execで委託する` / `Geminiに実装を委託する`)でも ERROR が出ることを実測する** — 空白区切りの治具だけでは単語境界の取りこぼしという穴を検出できない。あわせて `.codex/config.toml` を含む治具では ERROR が出ず exit=0 であること(誤検出の確認)、`SKILL.md` 以外の skill 直下ファイル(例 `README.md`)に語を置いた治具では ERROR が出ること(スコープが `*.md` や `SKILL.md` に絞られていないことの確認)も実測する。確認後は治具を削除し、`git status --short` に治具由来のエントリを残さない

出典: `docs/minutes/2026-09-17_壁打ち_implementerの外部委託.md`(決定 23・24)・Issue #54
