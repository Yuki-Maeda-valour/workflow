...
外側の while を skill にせず、同梱スクリプト `loop.sh` が回す(while を skill にすると 1 つのセッションの中で回り、「1 周 = 新しい文脈」が崩れる)。skill の数と責務は変えない。**実装は後続**(ship-task の入口と無人モード・完了条件の改竄ガード = #67〈#90 の K1・K5 の後〉/ `loop.sh`・ブレーカー・実 CLI の実動確認 = #68〈#81 の後〉/ 発見ループ = #69)。**時間切れの扱いと、worktree の元の ref は、決定録の暫定**(ユーザーの確認で確定する)。
...
- **止まったとき**: ship-task の無人モードが `保留_` に改名し、理由(どの停止条件か・人が次にすること)を追加修正記録に書き、**作業ブランチに commit する**(停止時はタスク MD を stage しない、という対話時の契約を読み替える)。報告点(§5-10)・許可の要る操作の拒否も `保留_` に落とす。周の時間上限は `loop.sh` が打ち切るだけで、時間切れの周と許可待ちで固まった周は、下の「隔離」のとおり worktree を残す(暫定)。`loop.sh` は改名しない
- **隔離**: 周ごとにローカルのデフォルトブランチから使い捨ての worktree を作り(fetch はしない。暫定)、その中で走らせる(人のチェックアウトに触れない)。周が正常に終わった(PR を開いたか、`保留_` を commit した)ときだけ worktree を消す。落ちた・時間切れ・未 commit が残った周は worktree を消さずに残し、そのタスクは以後拾わない。ループは次のタスクへ進み、連続する失敗はブレーカー(§4)で止める。残った worktree は朝の報告に並べる。**タスク MD はローカルのデフォルトブランチに commit 済み(task_dir が git の追跡下)であることが前提**で、追跡外なら `loop.sh` は起動時に止まる
...
features:
  ...
  loop:                      # 全項目フォールバック: キーが無い・値が無い項目は loop.sh の既定値を使う
    iteration_timeout: 3600  # 例。1 周の壁時計上限(秒)。超えた周は loop.sh が打ち切り、worktree を残して以後拾わない(§2。暫定)
...
20. **git 出口の規律**: ブランチ作成・commit・push・PR 作成を能動的に行うのは `/ship-task` のみ(`/do-task` はユーザーが `--branch` 等で明示指定したときだけブランチを作る。コミットは従来どおり求められた場合のみ。**求められていないのに `/do-task` が index に触れるのは、Phase 7 で追跡済みのタスク MD を `git mv` で改名するときのリネームの stage だけ**(未追跡のタスク MD は `mv` で改名し index に触れない。どちらも commit はしない)。**外部 implementer にはこの規律が及ばない** — 外部委託は規約の外にいる実行主体を生み、機構では git 操作を止められないため、委託プロンプトでの禁止と実行後の検証〈`external-runners.md` §12-3〉で扱う)。**マージは決して行わない**。PR は品質ゲートが緑・レビューが全員 APPROVED・実動確認が「実施済」か「実施不能」(§5-19)のときだけ開き、そうでないときはブランチと commit を残して停止する。commit メッセージ規約は `git log` から推定してプロジェクトに合わせ、実装と doc は別 commit に分ける。**無人ループの `loop.sh`(§2)は、ブランチ作成・commit・push・PR を行わない**。書き込みは周ごとの使い捨て worktree の作成・削除だけで、ブランチ作成・commit・push・PR は周の中の ship-task が行う(無人モードでは、止まったときの `保留_` への改名も commit する。§2)
