...
外側の while を skill にせず、同梱スクリプト `loop.sh` が回す(while を skill にすると 1 つのセッションの中で回り、「1 周 = 新しい文脈」が崩れる)。skill の数と責務は変えない。**実装は後続**(ship-task の入口と無人モード・完了条件の改竄ガード = #67〈#103 = #90 の K1・K5 の後〉/ `loop.sh`・ブレーカー・実 CLI の実動確認 = #68〈#81 の後〉/ 発見ループ = #69)。
...
- **1 周の中身**: ホスト CLI をヘッドレスで起動し、`/ship-task <タスク MD>` を無人モードで走らせる(Phase 1 を飛ばし、Phase 2 の続行判定から do-task → update-doc → PR)。周はブランチの無い worktree(detached HEAD)で始まるので、ship-task は **Phase 2 より前に**作業ブランチ `task/{名}` を作る。git の出口は ship-task のまま(§5-20)。**merge はしない**
- **拾う対象と順番**: タスク MD に無人実行のメタ行(名前は #67 で決める)がある `進行中_` だけ。`> **作成日**` の古い順(同じ日はファイル名順)。次のどちらかがあるタスクは拾わない(専用の状態ファイルは持たず、git の状態だけで判定する): 作業ブランチ `task/{名}`(ローカルかリモート。リモートは読み取りだけで確かめる。PR 待ち・保留中を含む)/ 前の周が残した worktree(落ちた周を含む)
- **止まったとき**: ship-task の無人モードが `保留_` に改名し、理由(どの停止条件か・人が次にすること)を追加修正記録に書き、**作業ブランチに commit する**(停止時はタスク MD を stage しない、という対話時の契約を読み替える)。報告点(§5-10)・許可の要る操作の拒否も `保留_` に落とす。周の時間上限は `loop.sh` が打ち切るだけで、時間切れの周と許可待ちで固まった周は、下の「隔離」のとおり worktree を残す。`loop.sh` は改名しない
...
1. **タスク保存先と命名**: 保存先は [`create-task/references/task-directory.md`](../plugins/dev-workflow/skills/create-task/references/task-directory.md) を正本として、管理プロジェクトルート基準で profile の `task_dir` → 既存状態名 MD の検出 → `docs/tasks` の順に解決する。`root` は本体ソース用で保存先の基準にはしない。ファイル名は `進行中_{タスク名}.md` → 完了時に同じディレクトリで `完了_{タスク名}.md`。中断は `中断_`、保留は `保留_`。無人ループ(§2)で ship-task が自分で止まったタスクは、ship-task の無人モードが `保留_` に改名し、理由を追加修正記録に書いて作業ブランチに commit する(時間切れ・許可待ちで固まった周は改名せず、worktree を残す。§2。完了条件の改竄や Phase 0 の 3 の停止など「失敗扱い」の周の落とし先は #67 で決め、worktree を残す側に落とすならこの規則の例外として書き足す)。発見ループが積む採用前の候補は `候補_`(正本・helper への追加は発見ループの実装時。[決定録 2026-09-23](minutes/2026-09-23_壁打ち_ループエンジニアリング.md))
...
14. **ログ永続化**: レビュー・検証の記録は `.claude/reviews/{role}-{タスク名}-iter{N}.md` に保存(タスク文脈を持たない skill は `{skill名}-iter{N}.md`)。**別のセッション・別の worktree の工程が判定に使う結果・印は、タスク MD(追加修正記録・ヘッダ)に残す** — create-task の設計レビューの行(ship-task の Phase 2 が読む)と、do-task の基準コミット行・追加修正記録(再開判定が読む)。`.claude/reviews/` のログはキャッシュで、在れば使う(§5-18 ①)。例外として、base-commit.md の新規着手の条件 ③ は、ログが有るときだけ保守側に倒す(無いときの判定は同じ。理由は base-commit.md)
...
20. **git 出口の規律**: ブランチ作成・commit・push・PR 作成を能動的に行うのは `/ship-task` のみ(`/do-task` はユーザーが `--branch` 等で明示指定したときだけブランチを作る。コミットは従来どおり求められた場合のみ。**求められていないのに `/do-task` が index に触れるのは、Phase 7 で追跡済みのタスク MD を `git mv` で改名するときのリネームの stage だけ**(未追跡のタスク MD は `mv` で改名し index に触れない。どちらも commit はしない)。**外部 implementer にはこの規律が及ばない** — 外部委託は規約の外にいる実行主体を生み、機構では git 操作を止められないため、委託プロンプトでの禁止と実行後の検証〈`external-runners.md` §12-3〉で扱う)。**マージは決して行わない**。PR は品質ゲートが緑・レビューが全員 APPROVED・実動確認が「実施済」か「実施不能」(§5-19)のときだけ開き、そうでないときはブランチと commit を残して停止する。commit メッセージ規約は `git log` から推定してプロジェクトに合わせ、実装と doc は別 commit に分ける。**無人ループの `loop.sh`(§2)は、ブランチ作成・commit・push・PR を行わない**。git への書き込みは周ごとの使い捨て worktree の作成・削除だけで、ブランチ作成・commit・push・PR は周の中の ship-task が行う(無人モードでは、止まったときの `保留_` への改名も commit する。§2)
