...
- **機密ガードの原則**: 宣言は「使ってよい」であって「毎回無確認で渡してよい」ではない。`secret_paths` が実在するプロジェクトでは明示確認を取ってから起動する。レビュー経路は読み取り専用であり、**ユーザーの git 状態(index・stash)を書き換えてはならない**。**外部 CLI は一時ツリー(作業ツリーの複製)で起動する**。cwd を限定できないランナーは既定で無効とし、一時ツリーを作れない構成では起動せず §5-9 に従って縮退と理由を報告する
