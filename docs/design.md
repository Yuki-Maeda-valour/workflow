...
- **退行防止は機械検査とレビュー観点の両方**(理由は下の「なぜそうするか」): `scripts/validate.py` に**移行済み skill の `SKILL.md` と `references/` に委託の語が無いこと**の検査を追加する(何をもって移行済みとするかの判別方法は実装時に定める。上の除外集合と `templates/` は検査対象から外す)。あわせてこのリポジトリの skill 変更レビューの観点に「ホスト結合の混入」を加える(§6)
- **版の刻み**: 基盤は 3.9.0(解決表が入ってから公開する)。移行は 3.9.x 系で刻み、**委託の語が全 skill の `SKILL.md` と `references/`(上の除外集合を除く)でゼロになった時点**(= 検査を全 skill 対象にして通る)で **v4.0.0** とする。移行途中の利用者は 3.9.x を使い続けても壊れない
