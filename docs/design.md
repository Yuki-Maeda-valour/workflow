...
**エイリアスの世代交代への追随**: 検査語彙のモデルエイリアスは**スナップショット**であり、世の中に新エイリアスが出たら**次の 4 箇所を同時に更新する**。① 上のコードブロックの測定コマンドの語彙 → ② `scripts/validate.py` の `_MODEL_ALIASES` → ③ 同 `FORBIDDEN_PATTERNS` の日付付きモデル ID → ④ 同 版数。**この順で直す**(design を先に直し、コードを追随させる。§5 の解決表と同じ向き)。⚠ **③④ から ① ② を導出しない** — ③④ は日付付き ID・版数を検出する正規表現で `mythos` を含み、**エイリアス集合の列挙ではない**。**導出はしないが同時に更新する**。⚠ **機械化しない**: 追随のトリガーが外部事象なので、① と ② を突き合わせる検査を足しても**両方が古いまま通る**。⚠ **§5-4 の「固定リストとして扱わない」とは別の話**(あちらは実行時に指定できるエイリアス集合)
