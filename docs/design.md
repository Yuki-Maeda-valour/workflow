...
14. **ログ永続化**: レビュー・検証の記録は `.claude/reviews/{role}-{タスク名}-iter{N}.md` に保存
...
- **description は「何をするか+いつ使うか(トリガー語句)」を日本語で 150〜350 字**。Claude の自動呼び出し判断の材料になるため、ユーザーが言いそうな表現(「〜して」)を含める。上限 1,024 字・description+when_to_use 合計 1,536 字
