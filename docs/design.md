...
24. **検証器(`scripts/validate.py`)の免除と可変語**:
    - **免除は明示マーカーだけ**: 禁止パターン検査の行単位の免除は、行内の `<!-- validate-allow: 理由 -->` に限る(**理由の記述が必須**)。行単位で効き、コードフェンスの内外は問わない。**効くのは禁止パターン検査だけ**で、委託の語(§7-7)・ホスト CLI 語(§7-7-1)・解決表の除外の不変条件は免除規則を持たない(そちらはマーカーでは消えず、役割語へ書き換えて消す)。以前は「禁止 / 使わない / しない / 廃止 / ではなく」を含む行を一律に免除していたが、普通の文に「〜しない」が入っているだけで混入が素通りしていた
...
- **対象語(限定を同じ文に置く: 「全域」ではなく検査範囲(下の「検査範囲」— skill 直下 + `references/` 配下の `*.md`、除外 2 本を除く)の実測が 0 件の語だけを入れる。§7 の測定コマンドと同じ考え方だが範囲が違う)**: `scripts/` / `templates/` を含めた全域では `cursor-agent` が **26 件**(`scripts/` 25 + `templates/` 1)・`gemini` が **6 件**(すべて `scripts/`)実在する(実測 2026-09-17)が、いずれも検査範囲外にあるため対象語として選べる。この 4 系統 9 語はランナー名(`cursor-agent` / `gemini`)・サンドボックスモード名(`workspace-write` / `danger-full-access`)・コマンド形(`codex exec` / `codex review` / `codex mcp`)・プラグイン/subagent 名(`codex-plugin-cc` / `codex-rescue`)
